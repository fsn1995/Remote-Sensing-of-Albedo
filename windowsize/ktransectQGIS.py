...
date_start = ee.Date.fromYMD(2020, 7, 19)
date_end = ee.Date.fromYMD(2020, 7, 23)
...
oliCol = ee.ImageCollection('LANDSAT/LC08/C02/T1_L2') \
            .filter(colFilter) \
            .map(prepOli) \
            .select(['Blue', 'Green', 'Red', 'NIR', 'visnirAlbedo']) # .select(['totalAlbedo']) or  .select(['viznirAlbedo'])
...
albedoImage = multiSat.select("visnirAlbedo").mean().updateMask(greenlandmask)
Map.addLayer(albedoImage, visParams, 'harmonized albedo')
Map.addLayer(multiSat.mean(), {
  'min': 0,
  'max': 1,

...
var s2Col = ee.ImageCollection('COPERNICUS/S2_SR') 
            ...
            .map(prepS2);
            // .select(['visnirAlbedo']); //# .select(['totalAlbedo']) or  .select(['visnirAlbedo'])
var multiSat = oliCol.merge(s2Col).sort('system:time_start', true);
print(multiSat);

Map.addLayer(oliCol.select('visnirAlbedo'), vis, 'landsat');
Map.addLayer(s2Col.select('visnirAlbedo'), vis, 'sentinel');